...
const ElementAxes = ({ sigma_x, sigma_y, tau_xy, theta_deg }) => {
    const size = 100; // Size of the element
    ...
    return (
        <svg width={2 * size} height={2 * size} viewBox={`-${size} -${size} ${2 * size} ${2 * size}`}>
            ...
            {/* Axes */}
            <line x1={-halfSize} y1="0" x2={halfSize} y2="0" stroke="blue" />
            <line x1="0" y1={-halfSize} x2="0" y2={halfSize} stroke="blue" />
            {/* Rotated axes */}
            <line x1={axisX.x1} y1={axisX.y1} x2={axisX.x2} y2={axisX.y2} stroke="red" />
            <line x1={axisY.x1} y1={axisY.y1} x2={axisY.x2} y2={axisY.y2} stroke="red" />
        </svg>
    );
};
